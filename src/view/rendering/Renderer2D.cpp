...
void Renderer2D::render(Packet* packet)
{
	...
	{
		cv::Mat frameDetailed = packet->getData().begin()->second;
		cv::namedWindow("Pose3D Cameras", cv::WINDOW_GUI_EXPANDED | cv::WINDOW_AUTOSIZE);
		cv::imshow("Pose3D Cameras", frameDetailed);
		cv::waitKey(1);
	}
	else
	{
		...
		cv::namedWindow("Pose3D Cameras", cv::WINDOW_GUI_NORMAL | cv::WINDOW_AUTOSIZE);
		cv::imshow("Pose3D Cameras", mergedImage);
		cv::waitKey(1);
	}

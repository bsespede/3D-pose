...
// Producer
DEFINE_string(scene_dir,                "scenes/test-scene",
    "Process a directory of a scene. Should have 3DPose format.");
...
namespace InputReader3D
{
	...
	InputPose3D::InputPose3D(std::string& scenePath)
	{
		...
		this->currentFrame = 120; // ROLL BAKEAR
		this->maxFrames = 0;
		readCaptureInfo();
	...
	void InputPose3D::readCaptureInfo()
	{
		...
		this->maxFrames = root.get<int>("capture.frames"); // ROLL BAKEAR
	}
